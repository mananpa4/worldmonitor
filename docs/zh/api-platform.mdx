---
title: "平台端点"
description: "引导、健康检查、版本管理、缓存清理和用户偏好设置 —— 每个 WorldMonitor 客户端都会调用的基础设施端点。"
---
...
- **认证**:浏览器 origin 或 `X-WorldMonitor-Key`
- **缓存**:`Cache-Control: public, max-age=30, s-maxage=30`
- **结构**:`{ earthquakes, outages, marketQuotes, commodityQuotes, imfMacro, bisPolicy, ... }` —— 约 40+ 个顶层字段,每个对应一个已 seed 领域的解包负载。
...
针对**所有已注册 seed 键**的聚合新鲜度报告。返回 `HEALTHY`、`WARNING`(过期)或 `CRIT`(缺失 / 为空且 `emptyDataIsFailure` 为真)。

通过 UptimeRobot / Better Stack 进行监控 —— 任何非 `HEALTHY` 状态都应触发告警。
