---
title: "贡献指南"
description: "WorldMonitor 项目的代码风格规范、Pull Request 流程、贡献类型以及安全实践。"
---
...
Serverless 代理函数会对所有参数进行校验与限幅：

| 端点 | 校验规则 |
|----------|------------|
| `/api/yahoo-finance` | 符号格式 `[A-Za-z0-9.^=-]`，最长 20 字符 |
| `/api/coingecko` | Coin ID 为字母数字加连字符，最多 20 个 ID |
| `/api/polymarket` | 排序字段白名单，limit 限制在 1-100 |
