---
title: "商务端点"
description: "结账、客户门户、产品目录和推荐。基于 Convex + Dodo Payments 的轻量边缘代理。"
---
...
**响应**（分层顺序为 `free`、`pro`、`api_starter`、`enterprise`）：
...
说明：
- 价格字段直接放在分层对象上。付费分层暴露 `monthlyPrice` / `monthlyProductId` 和 `annualPrice` / `annualProductId`。Free 使用 `price: 0, period: "forever"`；Enterprise 使用 `price: null`。
