---
title: "AI 情报"
description: "World Monitor 所使用的 LLM 链、RAG 流水线、威胁分类、推演引擎以及浏览器端 ML。"
---
...
**导出选项**：简报可导出为 JSON（包含所有评分、信号和标题的结构化数据）、CSV（扁平表格格式）或 PNG 图片。打印按钮会调用浏览器原生打印对话框以导出 PDF。
...
1. **关键词分类器**（即时，`source: 'keyword'`）—— 对约 120 个威胁关键词进行模式匹配，这些关键词按严重度层级（critical → high → medium → low → info）和 14 个事件类别（conflict、protest、disaster、diplomatic、economic、terrorism、cyber、health、environmental、military、crime、infrastructure、tech、general）组织。关键词使用词边界正则匹配以避免误报（例如 "war" 不会匹配 "award"）。每次匹配返回一个严重度级别、类别与置信度分数。变体专属的关键词集确保科技变体不会在非地缘政治语境下标记 "sanctions"。
...
仪表盘通过 Transformers.js 在浏览器内运行完整 ML 流水线，核心情报不依赖任何服务器。为节省内存，该功能在移动设备上自动禁用。
