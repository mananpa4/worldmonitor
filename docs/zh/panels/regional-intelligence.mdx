---
title: "区域情报"
description: "按区域的结构化快照 — 涵盖态势、平衡、参与者、场景、传导路径和关注列表 — 由 Intelligence service 的规范 RegionalSnapshot 支持。"
---
...
区域切换触发单次 RPC 调用（无轮询）。请求序列仲裁器防止用户快速切换区域时的过期渲染。
