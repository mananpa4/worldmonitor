...
description: "运行预构建的供应链中断场景 — 冲突、制裁、关税冲击和天气事件 — 直接在地图上查看哪些咽喉要道、板块和国家受影响。"
---

Scenario Engine 将 WorldMonitor 的实时供应链图转化为交互式 what-if 工具。你不再问"今天这条航线状态如何"，而是选择一个命名的中断场景 — 霍尔木兹海峡关闭、巴拿马干旱、半导体关税冲击 — 引擎解析对咽喉要道、HS2 板块和国家的下游影响，然后将结果绘制到现有地图上。
...
每个模板声明其影响的咽喉要道（来自咽喉要道注册表的 ID）、持续时间（天）、受影响的 HS2 板块和成本冲击乘数。按原样运行 — v1 中没有滑块。`ScenarioType` 联合类型为 `infrastructure` 和 `pandemic` 类别留有空间，但目前没有这些类型的模板。
...
- **受影响咽喉要道** — 哪些在地图上变红。
- **影响排名** — 按 ISO-2 排序的受影响最大的国家，按暴露度排序，附带按国家对所选 HS2 板块的进口值影响。
- **板块标注** — 哪些 HS2 章节受打击最大。
...
API 侧的速率限制 — 10 个作业/分钟/IP，全局队列上限 100 个进行中 — 记录在 [Scenarios API](/zh/api-scenarios#run-a-scenario) 中。
...
- **贸易/影响数据** — 按国家的 HS2 进口暴露，来自供应链缓存集。
