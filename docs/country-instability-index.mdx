...
historical baseline floor. If ACLED auth is unconfigured the conflict realtime
signal degrades to UCDP-only, which `/api/health.riskScores` surfaces as
`COVERAGE_PARTIAL` (see [Data Sources](/data-sources#dual-source-protest-tracking)).
